  ...
  module Adapters
    class GrokAdapter < Port
      ...
      def extract(file_bytes_or_io, content_type: "image/jpeg", options: {})
        api_key = credential
        if api_key.blank?
          return build_error_result("Credencial Grok (xAI) não configurada (defina a chave no banco ou em XAI_API_KEY)")
        end
        ...
        build_error_result("Erro de conexão com a API xAI Grok: #{e.message}")
      rescue StandardError => e
        build_error_result("Falha no processamento Grok: #{e.message}")
      end
